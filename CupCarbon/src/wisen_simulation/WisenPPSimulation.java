 ...
 * @version 1.0
 */
public class WisenPPSimulation extends Thread {
	...
	public void simulate() {
		...
		try {
			...
			WisenPPSemaphore wisenSemaphorePing = new WisenPPSemaphore(false) ;
			WisenPPSemaphore wisenSemaphorePong = new WisenPPSemaphore(true) ;
			
			WisenPPPing wisenPing = new WisenPPPing(wisenSemaphorePing, wisenSemaphorePong, this);
			WisenPPPong wisenPong = new WisenPPPong(wisenSemaphorePing, wisenSemaphorePong, this);
			wisenPing.start();
			wisenPong.start();	
			try {
				wisenPong.join();
			...
			if (mobility)
				if (event2[i] == 0) {
					if (deviceList.get(i).canMove()) {
						deviceList.get(i).moveToNext(visual, visualDelay);
